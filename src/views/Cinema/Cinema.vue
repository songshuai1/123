		...
		>
			<header class="header">
				<div class="left">
					...
					>
						<span @click="goRouter">{{ city | cleaner }}</span>
						<img
			...
			<div class="cinema-list-wrap">
				<ul class="cinema-list">
					<li
						...
						v-for="item in movieCinema"
						:key="item.cinemaId"
					>
						<div class="cinema-item-wrap" @click="gocinepleX(item.cinemaId)">
...
<script>
import { movieCinemaData, movieGatewayData } from "@/api/api";
...
export default {
	...
	async mounted() {
		this.data1 = this.$store.state.city;

		let ret = await movieCinemaData();
		this.movieCinema = ret.data.data.cinemas;
		let res = await movieGatewayData();
		this.movieGateway = res.data.data;
	...
	async updated() {
		if (this.movieCinema.length == 0 && this.movieGateway.length == 0) {
			this.loading = true;
		} else {
			this.loading = false;
		}
	...
	created() {
		if (this.$store.state.cityId === 0) {
			if (confirm("当前无法定位到您所在的城市,是否选择城市")) {
